...
function FloatingTimer({ seconds }) {
  ...
  const color  = isCrit ? '#ef4444' : seconds <= 15 ? '#fb923c' : '#22c55e';
  ...
  return (
    ...
    >
      <svg width="60" height="60" viewBox="0 0 60 60" style={{ position: 'absolute', top: 0, left: 0 }}>
        ...
        <circle
          cx="30" cy="30" r={r} fill="none" stroke={color} strokeWidth="3"
...
function CountdownRing({ seconds }) {
  ...
  const color   = isCrit ? '#ef4444' : isUrgent ? '#fb923c' : '#22c55e';
  ...
  return (
    <motion.div
      ...
      transition={isCrit ? { repeat: Infinity, duration: 0.4 } : {}}
    >
      <div className="ring-glow-outer" style={{ boxShadow: `0 0 80px 20px ${color}22` }} />
      <div className="ring-glow-mid"   style={{ boxShadow: `0 0 40px 10px ${color}33` }} />
      ...
      <svg width="280" height="280" viewBox="0 0 280 280" style={{ transform: 'rotate(-90deg)' }}>
        ...
        <circle cx="140" cy="140" r={r} fill="none"
          stroke={color} strokeWidth="14" strokeOpacity="0.12"
          strokeDasharray={`${pct * circ} ${circ}`} strokeLinecap="round" />
        {/* main arc */}
        <circle cx="140" cy="140" r={r} fill="none"
          stroke={color} strokeWidth="8"
          strokeDasharray={`${pct * circ} ${circ}`} strokeLinecap="round"
          style={{ transition: 'stroke-dasharray 0.6s ease, stroke 0.4s ease', filter: `drop-shadow(0 0 12px ${color})` }} />
      </svg>
  ...
  return (
    <div className="pot-amount">
      ...
      <span className="pot-number">
        {display > 0
          ? Number(display).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
          : '0.00'}
      </span>
    </div>
  );
...
/* ─── BUYER CARD ─────────────────────────────────────────────────────────── */
// entry = { wallet, role, pct, sol } — comes from stats/global.top10
function BuyerCard({ entry, isFirst }) {
  const pctStr = entry.pct != null ? `${(entry.pct * 100).toFixed(1)}%` : null;
  const solStr = entry.sol != null && entry.sol > 0 ? `${entry.sol.toFixed(4)} SOL` : null;
...
export default function App() {
  ...
  // This single document now contains lastBuyTime, currentPotSol,
  // currentPotUsd, and top10 — all written atomically by the engine
  useEffect(() => {
    return onSnapshot(doc(db, 'stats', 'global'), snap => {
      ...
      if (d.lastBuyTime) {
        nextRef.current = d.lastBuyTime.toMillis() + TIMER_TOTAL * 1000;
      }
    });
  }, []);
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      if (nextRef.current != null) {
        const secs = Math.ceil((nextRef.current - Date.now()) / 1000);
        setCountdown(Math.min(TIMER_TOTAL, Math.max(0, secs)));
      }
    }, 500);
    return () => clearInterval(iv);
  }, []);
    ...
    <div className={`app${isCritical ? ' app--critical' : isUrgent ? ' app--urgent' : ''}`}>
      ...
      >
        <div className="header-inner">
          <div className="header-brand">
            <img src="/logo.png" alt="Last Buyer Wins" className="brand-logo"
      ...
      <main className="main">
        ...
        <section className="hero">
          ...
          <motion.div className="hero-title-wrap"
            initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}>
            <img src="/logo.png" alt="Last Buyer Wins Logo" className="hero-logo"
              onError={e => { e.target.style.display = 'none'; }} />
          ...
          <motion.p className="hero-sub"
            ...
            Buy ${MIN_BUY_USD}+ of $LBW. Reset the {TIMER_TOTAL}-second timer. If nobody buys after you —
            creator rewards are <strong>sent directly to your wallet.</strong>
          </motion.p>

          <motion.div className="eligibility-badge"
            ...
            Top 10 buyers split the pot.
          </motion.div>

          <motion.div className="ca-wrap"
        ...
        <section className="arena">
          ...
          {/* Left: Timer + Pot + Stats */}
          <motion.div className="arena-timer"
            ...
            <div className="timer-header">
              ...
              {isUrgent && (
                <motion.span className="timer-urgent-badge"
                  animate={{ opacity: [1, 0.4, 1] }} transition={{ repeat: Infinity, duration: 0.6 }}>
          ...
          {/* Right: Live Feed */}
          <motion.div className="arena-feed"
                  ...
                  <motion.div key="buyers" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    {top10.length === 0 ? (
                      <div className="feed-empty">
                        ...
                        <div className="feed-empty-sub">Be the first — buy ${MIN_BUY_USD}+ and lead the countdown.</div>
                      </div>
                    ) : (
                      <>
        ...
        <section className="how">
          ...
          <div className="how-grid">
            {[
              { n: '01', icon: '💸', h: `Buy $${MIN_BUY_USD}+ of $LBW`,
                b: `Purchases under $${MIN_BUY_USD} are ignored. Only qualifying buys reset the timer and enter you into the top 10.` },
              { n: '02', icon: '⏱',  h: 'Timer resets',
                b: `Every qualifying buy resets the ${TIMER_TOTAL}-second countdown and moves you to the top of the leaderboard.` },
              { n: '03', icon: '🏆', h: 'Last buyers split the pot',
                b: 'When the timer hits zero: the last buyer gets 50%, and the remaining top 9 share the other 50% equally.' },
            ].map((c, i) => (
              <motion.div key={c.n} className="how-card"
      ...
      <footer className="footer">
        <div className="footer-inner">
          <div className="footer-brand">
            <img src="/logo.png" alt="Last Buyer Wins" className="footer-logo"
              onError={e => { e.target.style.display = 'none'; }} />
